...
class pdf extends FPDF {
  ...
  function SetStyle($tag, $family, $style, $size, $color, $fillcolor, $indent = -1) {
    $tag = trim($tag);
    $this->TagStyle[$tag]['family'] = trim($family);
    $this->TagStyle[$tag]['style'] = trim($style);
    $this->TagStyle[$tag]['size'] = trim($size);
    $this->TagStyle[$tag]['color'] = trim($color);
    $this->TagStyle[$tag]['fillcolor'] = trim($fillcolor);
  ...
  function DoStyle($tag) { // Applies a style
    ...
    $tab = explode(",", $this->TagStyle[$tag]['fillcolor']);
    if (count($tab) == 1)
      $this->SetFillColor($tab[0]);
    else
      $this->SetFillColor($tab[0], $tab[1], $tab[2]);
  }
  ...
  function FindStyle($tag, $ind) { // Inheritance from parent elements
    ...
    // FillColor
    if ($this->TagStyle[$tag]['fillcolor'] != "")
      $fillcolor = $this->TagStyle[$tag]['fillcolor'];
    else {
      reset($this->PileStyle);
      while (list($k, $val) = each($this->PileStyle)) {
        $val = trim($val);
        if ($this->TagStyle[$val]['fillcolor'] != "") {
          $fillcolor = $this->TagStyle[$val]['fillcolor'];
          break;
        }
      }
    }
    ...
    $this->TagStyle[$ind]['fillcolor'] = $fillcolor;
  ...
  function Parser($text) {
    ...
    // Opening tag
      ...
      preg_match_all("/\b(\w+=([\"'])[^\\2]+?\\2)/", $regs[2], $pairs);
      $pairs = $pairs[0];

      foreach ($pairs AS $pair) {
        $atr = preg_split("/\s*=\s*/", $pair);
        $end = strlen($atr[1]) - 2;

        $tab[$atr[0]] = substr($atr[1], 1, $end);
      }
  ...
  function PrintLine() {
    ...
    for ($i = 0; $i < count($this->Line2Print); $i++) {
      ...
      if (isset($this->TagHref[$i]))
        ...
        $href = '';
      if ($this->Line2Print[$i] == "") {
        $this->Cell($space, $this->hLine, "         ", 0, 0, 'C', true, $href);
      } else {
        $this->Cell($this->StringLength[$i], $this->hLine, $this->Line2Print[$i], 0, 0, 'C', true, $href);
      }
    }
